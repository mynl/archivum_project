...
class Document():
    ...
    def get_guess_crossref_query(self):
        """Try to guess a reasonable cross ref query search from the metadata and filename."""
    ...
    def add_title_author_from_cover(self):
        """
        ...
        try:
            self.cover_title =next(ititle)[1].strip() if texts else ""
            if self.cover_title.lower().startswith('arxiv'):
                self.cover_arxiv = self.cover_title.split(' ')[0]
                self.cover_title = next(ititle)[1].strip()
            self.cover_author = [next(ititle)[1].strip() for i in range(4)]
    ...
    def uber(self):
        ...
        self.stats;
        self.add_meta_data()
        self.add_title_author_from_cover()
        self.bg_title = self.get_best_guess_title()
        self.bg_crq = self.get_guess_crossref_query()
        self.bg_arxiv = getattr(self, 'cover_arxiv', "").lower().replace('arxiv:', "")

        print(self.bg_arxiv)

        if self.bg_arxiv != "":
            ans = lookup_arxiv([self.bg_arxiv])
            print(ans)
            self.arxiv_title = ans.get('title', "??")
            self.arxiv_author = ans.get('author', "??")
            self.arxiv_year = ans.get('year', "??")

    def assembled_df(self):
        self.uber()
        return pd.Series({ k: v for k, v in self.__dict__.items()  if isinstance(v, str)},
            name='text')
